        ...
        <section className="bg-gradient-to-br from-emerald-50 via-white to-emerald-100 min-h-[calc(100vh-6rem)] flex items-center justify-center px-4 sm:px-6 py-8">
            <div className="w-full max-w-md mx-auto rounded-2xl shadow-2xl bg-white p-6 sm:p-10">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    <h2 className="text-lg sm:text-2xl font-extrabold text-emerald-900">Create your FastCart account</h2>
                    <p className="text-sm text-slate-500">Get started with great deals and fast delivery</p>
                  </div>
                </div>
                ...
                <form className="grid gap-4 py-3 w-full" onSubmit={handleSubmit} noValidate>
                    <div className='grid gap-1'>
                        ...
                        <input
                            ...
                            autoFocus
                            className={`p-3 border rounded-md text-base sm:text-lg focus:outline-none ${errors.name ? 'border-red-300 bg-red-50' : 'border-emerald-100 bg-emerald-50'}`}
                            name='name'
                            value={data.name}
                            onChange={(e)=>{ handleChange(e); setErrors(prev=> ({...prev, name: undefined})) }}
                            placeholder='Your full name'
                        />
                        {errors.name && <div id="name-error" className="text-xs text-red-600 mt-1">{errors.name}</div>}
                    </div>
                    ...
                    <div className='grid gap-1'>
                        ...
                        <div className={`p-2 rounded-md flex items-center ${errors.confirmPassword ? 'border border-red-300 bg-red-50' : 'border border-emerald-100 bg-emerald-50'}`}>
                            ...
                            />
                            <button type="button" onClick={() => setShowConfirmPassword(preve => !preve)} aria-label={showConfirmPassword ? 'Hide password' : 'Show password'} className='text-slate-600 hover:text-slate-800 ml-2'>
